...
## 📚 Overview
Production-ready `AttendanceSystem` class yang mengintegrasikan file-based storage dengan face recognition untuk complete automated attendance system.
...
**Initializes:**
- File-based CSV storage (logs/attendance.csv)
- RecognitionService instance (loads from pickle)
...
### 1. add_employee() (uses DatasetManager)
**Purpose:** Register new employee in dataset

```python
# Uses DatasetManager from Week 4
...
**Parameters:**
- `name` (str): Full name
- `employee_id` (str): Unique employee identifier (optional)
- `department` (str): Department name (optional)

**Returns:** `str` - person_id (folder name)

**Creates:** Person folder in dataset/

---

### 2. register_employee_face() (uses DatasetManager)
...
# Interactive capture
manager.capture_faces(
    person_id='alice_johnson',
    target_count=30
)

# Generate encodings
manager.generate_encodings(model_name='Facenet512')
```

**Parameters:**
- `person_id` (str): Person identifier (folder name)
- `target_count` (int): Number of photos to capture (default: 20)

**Process:**
1. Opens webcam
2. Guides photo capture
3. Validates quality
4. Saves to person folder
5. Generates encodings to pickle
6. Updates metadata.json

**Returns:** `int` - photos_captured

---

### 3. mark_attendance_auto()
**Purpose:** Automated attendance marking via face recognition (camera)

```python
# Auto check-in mode
attendance.process_camera_attendance(
    camera_id=0,
    mode='check_in'
)
